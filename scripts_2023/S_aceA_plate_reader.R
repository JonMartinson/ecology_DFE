...
OD_lab2 <- OD_lab %>% 
    mutate(
        ...
        media_type = case_when(media_type == 'ace' ~ 'Acetate',
                          ...
                          media_type == 'spent' ~ 'Filtered Spent E Media')
    )


media_levels <- c('Galactose', 'Acetate', 'Filtered Spent E Media', 'Coculture with E')
...
B <- loglin_gr_meta %>% 
    ...
    labs(x = "", y = "Growth rate", color = 'Genotype')+
    facet_wrap(~ factor(media_type, levels = media_levels)) 
...
C <- loglin_gr_meta %>% 
    ...
    theme_bw(12)+
    theme(legend.position = 'none')+
    labs(x = "", y = "Max OD", color = 'Genotype')+
    facet_wrap(~ factor(media_type, levels = media_levels)) 
